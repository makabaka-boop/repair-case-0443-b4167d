...
`409 INCOMPLETE` 响应携带的成员缺口明细与裁决来自**同一份锁后快照**：失败原因
在锁释放后不会被重读改写。即使等待中的最后一片（或另一个组封存请求）在裁决之
后随即提交，响应列出的仍是裁决时刻真实缺失的成员与序号——失败响应永远自洽，
调用方能据此准确补片或重试，而不会拿到与数据库状态矛盾的"空"失败。
...
- `internal/store`：乱序、重传幂等、UTF-8 字节判同、冲突、封存不完整保持 OPEN、
  ...
  组裁决之后才提交的补片不得改写 INCOMPLETE 响应的缺口明细（确定性交错）、
  关闭全部连接后重开的重启一致性；
- `internal/api`：两个独立连接池支撑的两个 HTTP 实例上交替写入/封存、全状态码矩阵、
  ...
  组封存与最后一片跨实例竞争时 409 必含不完整成员（25 轮）、
  关闭并重建两个实例后的终态一致性；
