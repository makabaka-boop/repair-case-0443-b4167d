...
任一成员缺片则整体回滚，且 `409 INCOMPLETE` 携带的正是该锁后快照——即使排在
锁后的补片随后落库、甚至另一请求已封存整组，失败响应也不会被改写成与数据库
矛盾的形态（例如 `batches` 为空的 409）。因此 `[A,B]` 与 `[B,A]` 的反序组请求、
单批次封存与最后一片分片三方并发时只会串行等待、不会死锁，也不可能出现"部
分成员被封存"的中间态；已 `SEALED` 成员的 `sealedAt` 永不被覆盖。
...
- `internal/store`：乱序、重传幂等、UTF-8 字节判同、冲突、封存不完整保持 OPEN、
  ...
  （25 轮）、组请求锁等待期间提交的最后一片必须被组裁决看见（确定性交错）、
  组封存判缺片时排在锁后的补片不得改写 409 的缺口快照（确定性交错）、
  关闭全部连接后重开的重启一致性；
